...
import { MIN_WITHDRAW_POINTS, FEE_PERCENT_BPS, FEE_FIXED_POINTS, DAILY_CAP_POINTS, POINTS_PER_TOKEN, } from "../config.js";
...
export default async function walletRoutes(fastify) {
    ...
    fastify.post("/api/stake", async (req, reply) => {
        ...
        if (!address || !amount)
            return reply.status(400).send({ error: "address/amount required" });
    ...
    fastify.post("/api/unstake", async (req, reply) => {
        ...
        const u = await ensureUser(addr);
        if (u.staked < a)
            return reply.status(400).send({ error: "insufficient staked" });
        await prisma.user.update({
            where: { address: addr },
            data: {
                staked: { decrement: a },
    ...
    fastify.get("/api/withdraw/preview", async (req, reply) => {
        ...
        if (a <= 0n)
            return reply.status(400).send({ error: "invalid amount" });
        const u = await ensureUser(address.toLowerCase());
        if (u.balance < a)
            return reply.status(400).send({ error: "insufficient balance" });
        ...
        if (net <= 0n)
            return reply.status(400).send({ error: "net is zero" });
    ...
    fastify.post("/api/withdraw", async (req, reply) => {
        const { address, amount } = req.body;
        if (!address || !amount)
            return reply.status(400).send({ error: "address/amount required" });
        const a = BigInt(amount);
        ...
        // min & balance
        if (a < MIN_WITHDRAW_POINTS)
            return reply.status(400).send({ error: `min ${MIN_WITHDRAW_POINTS}` });
        if (u.balance < a)
            return reply.status(400).send({ error: "insufficient balance" });
        // daily cap
        ...
        const w = await prisma.$transaction(async (tx) => {
            await tx.user.update({
                where: { id: u.id },
                data: { balance: { decrement: a } }, // reserve
            });
    ...
    fastify.get("/api/users/:address/withdrawals", async (req, reply) => {
        ...
        if (!u)
            return toJSONBigInt({ ok: true, items: [] });
