...
export default async function adminExtendedRoutes(fastify: FastifyInstance) {
  // Apply admin auth to all routes
  fastify.addHook("onRequest", verifyAdmin);

  // GET /api/admin/users - Get all users with pagination
  fastify.get("/api/admin/users", async (req, reply) => {
    const { page = "1", limit = "50", search = "", sortBy = "createdAt", sortOrder = "desc" } = req.query as {
      page?: string;
      limit?: string;
      search?: string;
      sortBy?: string;
  ...
  fastify.post("/api/admin/users/:id/ban", async (req, reply) => {
    ...
    // For now, we'll use a workaround or return success
  ...
  fastify.get("/api/admin/contributions", async (req, reply) => {
    const { page = "1", limit = "50", status = "all" } = req.query as {
      page?: string;
      limit?: string;
      status?: string;
    };
  ...
  fastify.get("/api/admin/stats", async (req, reply) => {
    const [totalUsers, totalContributions, totalBurned, activeEvents] = await Promise.all([
      ...
      prisma.$queryRaw`SELECT COUNT(*) as count FROM "Event" WHERE "endsAt" > NOW() OR "endsAt" IS NULL` as Promise<[{ count: bigint }]>,
    ]);
