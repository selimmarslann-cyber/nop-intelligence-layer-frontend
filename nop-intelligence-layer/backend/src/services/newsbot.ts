...
type Item = { title: string; link: string; source: string; isoDate: string | undefined };

function uniqByTitle(items: Item[]): Item[] {
  const m = new Map<string, Item>();
  for (const it of items) if (it.title) m.set(it.title, it);
 ...
 */
export async function fetchAndPostNews(): Promise<{ saved: number; skipped: number }> {
  const all: Item[] = [];
